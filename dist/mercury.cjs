...
var require_difflib = __commonJS({
  "node_modules/.pnpm/difflib@git+https+++git@github.com+postlight+difflib.js.git#32e8e38c7fcd935241b9baab71bb432fd9b166ed/node_modules/difflib/lib/difflib.js"(exports2) {
    init_cjs_shims();
    (function() {
...
var require_difflib2 = __commonJS({
  "node_modules/.pnpm/difflib@git+https+++git@github.com+postlight+difflib.js.git#32e8e38c7fcd935241b9baab71bb432fd9b166ed/node_modules/difflib/index.js"(exports2, module2) {
    init_cjs_shims();
...
var import_cheerio6 = __toESM(require("cheerio"));
  ...
  content: {
    ...
    transforms: {
      "img[data-original]": ($node) => {
        ...
        const src = $node.attr("src");
        const url = import_url.default.resolve(src, dataOriginal);
        $node.attr("src", url);
      }
    },
...
init_cjs_shims();
var import_cheerio3 = __toESM(require("cheerio"));
var import_string_direction = __toESM(require("string-direction"));
...
function markToKeep(article, $, url, tags = []) {
  ...
  if (url) {
    const { protocol, hostname } = import_url2.default.parse(url);
...
function articleBaseUrl(url, parsed) {
  const parsedUrl = parsed || import_url3.default.parse(url);
...
function getEncoding(str) {
  ...
  if (import_iconv_lite.default.encodingExists(str)) {
    encoding = str;
  }
  return encoding;
}
...
function absolutize($, rootUrl, attr) {
  ...
  $(`[${attr}]`).each((_, node) => {
    ...
    if (!url) return;
    const absoluteUrl = import_url4.default.resolve(baseUrl || rootUrl, url);
  ...
  $("[srcset]", $content).each((_, node) => {
    ...
    if (urlSet) {
      ...
      const absoluteCandidates = candidates.map((candidate) => {
        ...
        parts[0] = import_url4.default.resolve(rootUrl, parts[0]);
        return parts.join(" ");
      });
...
init_cjs_shims();
var import_valid_url = __toESM(require("valid-url"));
function clean(leadImageUrl) {
  leadImageUrl = leadImageUrl.trim();
  if (import_valid_url.default.isWebUri(leadImageUrl)) {
    return leadImageUrl;
  }
  return null;
}
...
function createDate(dateString, timezone, format) {
  if (TIME_WITH_OFFSET_RE.test(dateString)) {
    return (0, import_moment_timezone.default)(new Date(dateString));
  }
  if (TIME_AGO_STRING.test(dateString)) {
    const fragments = TIME_AGO_STRING.exec(dateString);
    return (0, import_moment_timezone.default)().subtract(fragments[1], fragments[2]);
  }
  if (TIME_NOW_STRING.test(dateString)) {
    return (0, import_moment_timezone.default)();
  }
  return timezone ? import_moment_timezone.default.tz(dateString, format || (0, import_moment_parseformat.default)(dateString), timezone) : (0, import_moment_timezone.default)(dateString, format || (0, import_moment_parseformat.default)(dateString));
}
...
var import_wuzzy = __toESM(require("wuzzy"));
...
function cleanDomainFromTitle(splitTitle, url) {
  const { host } = import_url5.default.parse(url);
  const nakedDomain = host.replace(DOMAIN_ENDINGS_RE, "");
  const startSlug = splitTitle[0].toLowerCase().replace(" ", "");
  const startSlugRatio = import_wuzzy.default.levenshtein(startSlug, nakedDomain);
  if (startSlugRatio > 0.4 && startSlug.length > 5) {
    return splitTitle.slice(2).join("");
  }
  const endSlug = splitTitle.slice(-1)[0].toLowerCase().replace(" ", "");
  const endSlugRatio = import_wuzzy.default.levenshtein(endSlug, nakedDomain);
...
var GenericContentExtractor = {
  ...
  extract({ $, html, title, url }, opts) {
    ...
    $ = $ || import_cheerio.default.load(html);
...
function shouldScore(href, articleUrl, baseUrl, parsedUrl, linkText, previousUrls) {
  ...
  const { hostname: linkHost } = import_url6.default.parse(href);
...
function scoreLinks({
  ...
  parsedUrl = parsedUrl || import_url7.default.parse(articleUrl);
...
var GenericNextPageUrlExtractor = {
  extract({ $, url, parsedUrl, previousUrls = [] }) {
    parsedUrl = parsedUrl || import_url8.default.parse(url);
...
function parseDomain(url) {
  const parsedUrl = import_url9.default.parse(url);
  const { hostname } = parsedUrl;
  return hostname;
}
function result(url) {
...
init_cjs_shims();
var import_ellipsize = __toESM(require("ellipsize"));
...
function clean2(content, $, maxLength = 200) {
  ...
  return (0, import_ellipsize.default)(content, maxLength, { ellipse: "&hellip;" });
}
...
init_cjs_shims();
var import_cheerio2 = __toESM(require("cheerio"));
var getWordCount = (content) => {
  const $ = import_cheerio2.default.load(content);
...
var GenericExtractor = {
  ...
  direction: ({ title }) => import_string_direction.default.getDirection(title),
  extract(options) {
    const { html, $ } = options;
    if (html && !$) {
      const loaded = import_cheerio3.default.load(html);
      options.$ = loaded;
    }
...
function getExtractor(url, parsedUrl, $) {
  parsedUrl = parsedUrl || import_url10.default.parse(url);
  const { hostname } = parsedUrl;
...
init_cjs_shims();
var import_cheerio5 = __toESM(require("cheerio"));
...
var import_postman_request = __toESM(require("postman-request"));

// src/resource/utils/constants.js
init_cjs_shims();
var import_cheerio4 = __toESM(require("cheerio"));
var REQUEST_HEADERS = import_cheerio4.default.browser ? {} : {
...
function get(options) {
  return new Promise((resolve, reject) => {
    (0, import_postman_request.default)(options, (err, response, body) => {
      if (err) {
        reject(err);
      } else {
...
async function fetchResource(url, parsedUrl, headers = {}) {
  parsedUrl = parsedUrl || import_url11.default.parse(encodeURI(url));
  const options = {
    url: parsedUrl.href,
...
var Resource = {
  ...
  encodeDoc({ content, contentType, alreadyDecoded = false }) {
    if (alreadyDecoded) {
      return import_cheerio5.default.load(content);
    }
    const encoding = getEncoding(contentType);
    let decodedContent = import_iconv_lite2.default.decode(content, encoding);
    let $ = import_cheerio5.default.load(decodedContent);
    const contentTypeSelector = import_cheerio5.default.browser ? "meta[http-equiv=content-type]" : "meta[http-equiv=content-type i]";
    ...
    if (metaContentType && properEncoding !== encoding) {
      decodedContent = import_iconv_lite2.default.decode(content, properEncoding);
      $ = import_cheerio5.default.load(decodedContent);
    }
    return $;
  }
};
...
var Parser = {
  async parse(url, { html, ...opts } = {}) {
    ...
    if (!url && import_cheerio6.default.browser) {
      url = window.location.href;
      html = html || import_cheerio6.default.html();
    }
    const parsedUrl = import_url12.default.parse(url);
  ...
  browser: !!import_cheerio6.default.browser,
